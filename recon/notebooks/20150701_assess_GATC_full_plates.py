...
CAPILLARY_SAMPLES_FN = '/data/plasmodium/pfalciparum/recon/data/processed/capillary/initial_GATC_full_plates.xlsx'
!mkdir -p /data/plasmodium/pfalciparum/recon/data/processed/capillary

BAM_FILES_DIR = '/data/plasmodium/pfalciparum/recon/data/processed/capillary/bams/initial_GATC_full_plates'
GATC_BAM_FILES_FOFN = '/data/plasmodium/pfalciparum/recon/data/processed/capillary/bams/initial_GATC_full_plates/gatc_bams.%s.list'
!mkdir -p {BAM_FILES_DIR}

VARIANT_FILES_DIR = '/data/plasmodium/pfalciparum/recon/data/processed/capillary/variants/initial_GATC_full_plates'
...
SAMPLES_FN = DATA_DIR + '/%s_capillary_samples_initial_plate.txt'

PLOT_DIR = '/Users/rpearson/Documents/projects/Recon/Slides/20150701_assess_GATC_full_plates'
!mkdir -p {PLOT_DIR}
...
plt.savefig('%s/test.pdf' % PLOT_DIR, format='pdf')
...
tbl_capillary_samples.addfield('Covers full sequence', lambda rec: rec['forward'] + rec['reverse'] >= 1012).valuecounts('Covers full sequence')
...
snpeff_exe = 'java -jar /Users/rpearson/src/github/malariagen/methods-dev/pf3k_techbm/opt/snpeff/snpEff/snpEff.jar'

# <codecell>

input_vcf = GATC_CAPILLARY_VCF_FN % '2'
output_vcf = input_vcf.replace('.vcf', '.ann.vcf')
!{snpeff_exe} -v -o gatk Pf3D7v91 {input_vcf} > {output_vcf}
...
for callset in ['GATC']:
    output_fn = "%s/pgv4_gatk_subset_to_kelch13_initial_plate_%s.vcf.gz" % (DATA_DIR, callset)
...
etl.fromvcf("%s/pgv4_gatk_subset_to_kelch13_initial_plate_GATC.vcf.gz" % (DATA_DIR))

# <codecell>

tbl_illumina_calls = (etl
    .fromvcf("%s/pgv4_gatk_subset_to_kelch13_initial_plate_GATC.vcf.gz" % (DATA_DIR))
    .vcfmeltsamples()
    .vcfunpackcall()
...
for callset in ['GATC']:
    tbl_illumina_calls[callset] = (etl
        .fromvcf("%s/pgv4_gatk_subset_to_kelch13_initial_plate_%s.vcf.gz" % (DATA_DIR, callset))
        .vcfmeltsamples()
        .vcfunpackcall()
        .cut(['CHROM', 'POS', 'REF', 'ALT', 'SAMPLE', 'AD'])
        .addfield('variant', lambda rec: "%s__%s__%s__%s" % (rec['CHROM'], rec['POS'], rec['REF'], rec['ALT']))
