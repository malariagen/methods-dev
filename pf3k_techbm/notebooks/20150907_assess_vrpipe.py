...
PLOTS_DIR = '/Users/rpearson/Documents/projects/Pf3k_techbm/slides/20150907_vrpipe_assessment/plots'
!mkdir -p {PLOTS_DIR}
...
def determine_mode(rec):
    if len(rec['REF']) == len(str(rec['ALT'][0])):
        return('SNP')
    else:
        return('INDEL')
...
(tbl_vrpipe_comparison_7g8_Pf3D7_01_v3
    .select(lambda rec: rec['IsInVRPIPE']==False and rec['IsInGATK']==True)
    .valuecounts('gatk_RegionType', 'GATK_mode', 'gatk_SNPEFF_EFFECT')
    .displayall()
)
...
(tbl_vrpipe_comparison_7g8_Pf3D7_01_v3
    .select(lambda rec: rec['IsInVRPIPE']==True and rec['IsInGATK']==True)
    .valuecounts('vrpipe_FILTER', 'gatk_FILTER')
    .displayall()
)
